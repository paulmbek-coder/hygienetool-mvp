...
export default function ErhitzungProdukte() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const data = JSON.parse(localStorage.getItem("erhitzungProdukte") || "[]");
    data.push(entry);
    localStorage.setItem("erhitzungProdukte", JSON.stringify(data));
    setSaved(true);
  ...
  return (
    <div className="min-h-screen p-6 flex justify-center bg-gray-50 fade-in">
      <form onSubmit={handleSubmit} className="card w-full max-w-2xl space-y-4">
        <h1 className="h1 text-center">🌡️ Erhitzung – Produkte</h1>

        <input className="input" type="date" name="datum" value={entry.datum} onChange={handleChange} />
        <input className="input" placeholder="Produkt" name="produkt" value={entry.produkt} onChange={handleChange} />
        <input className="input" placeholder="Gemessene Temperatur (°C)" name="temperatur" value={entry.temperatur} onChange={handleChange} />
